...
export default function ReceiptsPage() {
  ...
  const confirmM = useMutation({
    ...
    onSuccess: () => { 
      ['receipts', 'stock', 'stats', 'ledger'].forEach(k => qc.invalidateQueries({ queryKey: [k] }));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>Receipts</h1>
          <p style={{ color: 'var(--muted)', fontSize: '14px', margin: '4px 0 0' }}>Goods received from suppliers</p>
        </div>
      ...
      <div style={S.card}>
        {isLoading ? <div style={{ padding: '48px', textAlign: 'center', color: 'var(--muted)' }}><Loader2 size={24} style={{ animation: 'spin 1s linear infinite' }} /></div> : (
              ...
              {receipts.map((r) => (
                <>
                  <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => setExpanded(expanded === r.id ? null : r.id)}>
                    ...
                    <td style={{ ...S.td, color: 'var(--muted)' }}>{new Date(r.createdAt).toLocaleDateString()}</td>
                    <td style={S.td}>
                      {r.status === 'DRAFT' && (
                  ...
                  {expanded === r.id && (
                    <tr key={`${r.id}-exp`}><td colSpan={5} style={{ padding: 0, background: 'var(--surface)' }}>
                      <div style={{ padding: '12px 24px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        {r.lines.map((l, i) => (
                          <div key={i} style={{ fontSize: '13px', color: 'var(--foreground)' }}>• {l.product.name} — <b>{l.quantity}</b> units</div>
                        ))}
                      </div>
                    </td></tr>
                  )}
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: 'var(--card)', borderRadius: '12px', padding: '28px', width: '500px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: 700, color: 'var(--foreground)' }}>New Receipt</h2>
            <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ fontSize: '13px', fontWeight: 600, color: 'var(--foreground)', display: 'block', marginBottom: '6px' }}>Supplier</label>
                <input {...register('supplier')} placeholder="Supplier name" style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box' }} />
              </div>
              <div>
                ...
                {lines.map((l, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '8px', marginBottom: '8px' }}>
                    <select value={l.productId} onChange={(e) => updateLine(i, 'productId', e.target.value)}
                      style={{ padding: '9px 12px', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '14px' }}>
                      <option value="">Select product</option>
                      {products.map((p) => <option key={p.id} value={p.id}>{p.name} ({p.sku})</option>)}
                    </select>
                    <input type="number" min="1" value={l.quantity} onChange={(e) => updateLine(i, 'quantity', +e.target.value)}
                      style={{ width: '80px', padding: '9px 12px', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '14px' }} />
                  </div>
                ))}
              </div>

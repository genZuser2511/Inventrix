  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#111827', margin: 0 }}>Deliveries</h1>
          <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0' }}>Outgoing delivery orders</p>
        </div>
              ...
              {deliveries.map((d) => (
                <>
                  <tr key={d.id} onClick={() => setExpanded(expanded === d.id ? null : d.id)} style={{ cursor: 'pointer' }}>
                    ...
                    <td style={{ ...S.td, color: '#6B7280' }}>{new Date(d.createdAt).toLocaleDateString()}</td>
                  </tr>
                  {expanded === d.id && (
                    <tr key={`${d.id}-exp`}><td colSpan={4} style={{ padding: 0, background: '#F9FAFB' }}>
                      <div style={{ padding: '12px 24px' }}>
                        {d.lines.map((l, i) => <div key={i} style={{ fontSize: '13px', color: '#374151' }}>• {l.product.name} — <b>{l.quantity}</b> units</div>)}
                      </div>
                    </td></tr>
                  )}
                </>
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '12px', padding: '28px', width: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: 700, color: '#111827' }}>New Delivery Order</h2>
            <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ fontSize: '13px', fontWeight: 600, color: '#374151', display: 'block', marginBottom: '6px' }}>Customer</label>
                <input {...register('customer')} placeholder="Customer name" style={{ width: '100%', padding: '9px 12px', border: '1px solid #E5E7EB', borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box' }} />
              </div>
              <div>
